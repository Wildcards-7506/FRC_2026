...
public class DriveSubsystem extends SubsystemBase {
    ...
    public void holdSavedPose() {
        final double kP = 3.0;
        final double maxOut = 1.0;

        Pose2d curr = getPose();
        double xErr = savedPose.getX() - curr.getX();
        double yErr = savedPose.getY() - curr.getY();

        double xSpeed = MathUtil.clamp(xErr * kP, -maxOut, maxOut);
        double ySpeed = MathUtil.clamp(yErr * kP, -maxOut, maxOut);

        // drive(xParam, yParam, rot, fieldRel):
        //   xSpeedDelivered = yParam * kMax  →  pass xSpeed as yParam
        //   ySpeedDelivered = -xParam * kMax →  pass -ySpeed as xParam
        drive(-ySpeed, xSpeed, 0, true);

        SmartDashboard.putNumber("Hold X Err", xErr);
        SmartDashboard.putNumber("Hold Y Err", yErr);
        SmartDashboard.putNumber("Hold X CurrPose", curr.getX());
        SmartDashboard.putNumber("Hold Y CurrPose", curr.getY());
        SmartDashboard.putNumber("Hold X SavedPose", savedPose.getX());
        SmartDashboard.putNumber("Hold Y SavedPose", savedPose.getY());
    }
